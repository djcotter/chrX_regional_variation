...
parser.add_argument("--chrX_windows", action='store_true', help="This flag" +
                    ...
                    " for this analysis.")
parser.add_argument("--permute_chrX", nargs='?', default=False, type=int,
                    help="This can be used with the # of reps to return" +
                    " a P-value generated by using a permutation analysis n" +
                    " times. n can be provided by this flag. This must be an" +
                    " integer")
...
if args.chrX_windows is True:
    ...
    for w in wc:
        ...
        if args.permute_chrX is not False:
            w.append(pi_vals)  # index 6

    # if the permute flag is thrown, conduct a permutation test for sig.
    if args.permute_chrX is not False:
        reps = args.permute_chrX
        PAR1_count = 0
        PAR2_count = 0
        XTR_count = 0
        sites = [x for sublist in wc for x in sublist[6]]
        for i in range(reps):
            # PAR1 sample
            if sum(sample(sites, wc[0][5])) / wc[0][4] >= wc[0][3] / wc[0][4]:
                PAR1_count += 1
            # PAR2 sample
            if sum(sample(sites, wc[4][5])) / wc[4][4] >= wc[4][3] / wc[4][4]:
                PAR2_count += 1
            # XTR sample
            if sum(sample(sites, wc[2][5])) / wc[2][4] >= wc[2][3] / wc[2][4]:
                XTR_count += 1
        # divide the number of times that the regiosn were greater by # of reps
        PAR1_p = PAR1_count / reps
        PAR2_p = PAR2_count / reps
        XTR_p = XTR_count / reps

    # format the data array and add the two nonPAR sections together
    if args.permute_chrX is not False:
        # format is region, start, end, pi, called_sites, num variants, p-val
        data.append([wc[0][0], wc[0][1], wc[0][2], float(wc[0][3] / wc[0][4]),
                     wc[0][4], wc[0][5], PAR1_p])
